...
lok PROC uses eax ebx ecx esi, lokx:dword, loky:dword, lokf:dword
...
#define T0_0 "\n{0} PROC \n ;N\n ;E\n\n\n  call ExitProcess\n {0} ENDP\n\n"
#define T0_1 "\n{0} PROC uses eax ebx ecx edi esi;F\n ;N\n ;E\n  ret {1}\n{0} ENDP \n\n ;S\n"
#define T0_2 "\n{0} PROC uses eax ebx ecx edi esi;F\n ;E\n  ret {1}\n{0} ENDP  \n\n"
#define T0_3 "\n{0} PROC uses eax ebx ecx edi esi;F\n ;N\n ;E\n  ret {1}\n{0} ENDP \n\n"
#define T0_4 "\n{0} PROC uses eax ebx ecx edi esi;F\n ;E\n  ret {1}\n{0} ENDP\n;S\n\n"
#define T0_5 "\n{0} PROC \n ;N\n ;E\n\n\n  call ExitProcess\n{0} ENDP\n;S\n\n"
...
#define T3_0_INT ", {0}:sdword\n\n"
#define T3_1_INT ",{0}:sdword ;F"
#define T3_0_STR ", {0}:dword\n\n"
#define T3_1_STR ",{0}:dword ;F"
	...
	/*параметры функции*/
		case 3:
		{
			...
			case 0:
				...
				tmp.insert(firstOfNoTerminal, GEN1(T3_0_INT, lex.idtable.table[lex.lextable.table[mfst.deducation.lp[i]+1].idxTI].id));
				else 
				tmp.insert(firstOfNoTerminal, GEN1(T3_0_STR, lex.idtable.table[lex.lextable.table[mfst.deducation.lp[i] + 1].idxTI].id));


				break;
			case 1:
				if (lex.idtable.table[lex.lextable.table[mfst.deducation.lp[i] + 1].idxTI].iddatatype == IT::INT)
					tmp.insert(firstOfNoTerminal, GEN1(T3_1_INT, lex.idtable.table[lex.lextable.table[mfst.deducation.lp[i] + 1].idxTI].id));
				else
					tmp.insert(firstOfNoTerminal, GEN1(T3_1_STR, lex.idtable.table[lex.lextable.table[mfst.deducation.lp[i] + 1].idxTI].id));
				break;
			}
			break;
		}
